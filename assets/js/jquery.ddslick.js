(function (a) {
    function g(a, b) {
        ...
        if (l.showSelectedHTML) {
            d.html((m.imageSrc ? '<img class="dd-selected-image' + (l.imagePosition == "right" ? " dd-image-right" : "") + '" src="' + m.imageSrc + '" width="32" height="32" />' : "") + (m.text ? '<span class="dd-selected-text">' + m.text + "</span>" : "") + (m.description ? '<small class="dd-selected-description dd-desc' + (l.truncateDescription ? " dd-selected-description-truncated" : "") + '" >' + m.description + "</small>" : ""))
        } else d.html(m.text);
        e.val(m.value);
    ...
    var b = {},
        ...
        e = '<ul class="dd-options" style="z-index:9999"></ul>',
        f = '<style id="css-ddslick" type="text/css">' + ".dd-select{ border-radius:2px; border:solid 1px #ccc; position:relative; cursor:pointer;}" + ".dd-desc { color:#aaa; display:block; overflow: hidden; font-weight:normal; line-height: 1.4em; }" + "a.dd-selected{ overflow:hidden; display:block; padding:10px; font-weight:bold;text-decoration:none}" + ".dd-pointer{ width:0; height:0; position:absolute; right:10px; top:50%; margin-top:-3px;}" + ".dd-pointer-down{ border:solid 5px transparent; border-top:solid 5px #000; }" + ".dd-pointer-up{border:solid 5px transparent !important; border-bottom:solid 5px #000 !important; margin-top:-8px;}" + ".dd-options{ border:solid 1px #ccc; border-top:none; list-style:none; box-shadow:0px 1px 5px #ddd; display:none; position:absolute; z-index:2000; margin:0; padding:0;background:#fff; overflow:auto;}" + ".dd-option{ padding:10px; display:block; border-bottom:solid 1px #ddd; overflow:hidden; text-decoration:none; color:#333; cursor:pointer;-webkit-transition: all 0.25s ease-in-out; -moz-transition: all 0.25s ease-in-out;-o-transition: all 0.25s ease-in-out;-ms-transition: all 0.25s ease-in-out; }" + ".dd-options li {float:left;width:223px;}" + ".dd-options > li:last-child > .dd-option{ border-bottom:none;}" + ".dd-option:hover{ background:#f3f3f3; color:#000;}" + ".dd-selected-description-truncated { text-overflow: ellipsis; white-space:nowrap; }" + ".dd-option-selected { background:#f6f6f6; }" + ".dd-option-image {width:195px;margin:2px;border:1px solid #999;} " + ".dd-selected-image { vertical-align:middle; float:left; margin-right:5px; max-width:64px;border:1px solid #999}" + ".dd-image-right { float:right; margin-right:15px; margin-left:5px;}" + ".dd-container{ position:relative;}​ .dd-selected-text { font-weight:bold}​</style>";
        ...
        return this.each(function () {
            ...
            if (!f) {
                ...
                a.each(b.data, function (a, c) {
                    ...
                    m.append("<li>" + '<a class="dd-option" rel="tooltip" title="'+ (c.text? c.text:'') +'">' + (c.imageSrc ? ' <img class="dd-option-image' + (b.imagePosition == "right" ? " dd-image-right" : "") + '" src="' + c.imageSrc + '" />' : "")  + "</a>" + "</li>")
                });
                var n = {
                    settings: b,
                    original: k,
                ...
                if (b.hiddenInputName){
                    c.find(".dd-selected-value").prop('name', b.hiddenInputName);
                }
            }
        })
    };
