(function ($) {
    $.fn.tokenField = function (options) {
                        ...
                        if (settings.max == 0 || $(this).closest('.token-input').siblings('.token').length < settings.max) {
                            if (isToken($(this).val())) {
                                ...
                                if (!settings.allowDuplicates) {
                                    allowed = !isDuplicated($(this).val());
                                }
                                if (allowed) {
                                    var token = $(tokenHtml($(this).attr('name'), $(this).val()));

                                    observeToken(token.insertBefore($(this).closest('.token-input')));
                                    if (!settings.nested) {
                                        var input = $(this).closest('.token-field').find('input:hidden');
                                        var values = input.val().length ? input.val().split(',') : [];
                                    ...
                                    if ($.isFunction(settings.added)) {
                                        settings.added(token);
                                    }
                                }
                                else {
                                    if($.isFunction(settings.duplicated))
                                    {
                                        settings.duplicated($(this).val());
                                    }
                                }
                                $(this).val('');
                            } else {
        ...
        {
            tokenList = $.map(tokenList, function(val){
        ...
        function isDuplicated(text) {
            return $.inArray(text, tokenList) > -1;
        }
...
//        console_log('Warning: ignoring bad token - '+v);
        ...
        function removeToken(token) {
            if (!settings.nested) {
                ...
                var text = values.splice(index, 1);
                removeFromList(text);
                input.val((values.length?values.join(','):''));
            }
            token.remove();
            if ($.isFunction(settings.removed)) {
                settings.removed(token);
            }
        }

        function tokenHtml(name, text) {
            return "<a href='#' class='token'><span><span><span><span class='token-text'>"
                + text
